...
public class XXFRatingBar extends androidx.appcompat.widget.AppCompatRatingBar {
    ...
    public XXFRatingBar(Context context) {
        this(context, null);
    }

    public XXFRatingBar(Context context, AttributeSet attrs) {
        // notice:can't use this(context, attrs, 0); because ratingbar has it's own defStyleAttr
        super(context, attrs);
        init(context, attrs, 0);
    }

    public XXFRatingBar(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                R.styleable.AndRatingBar, defStyleAttr, 0);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.XXFRatingBar, defStyleAttr, 0);
        right2Left = typedArray.getBoolean(R.styleable.XXFRatingBar_right2Left, false);

        if (typedArray.hasValue(R.styleable.XXFRatingBar_starColor)) {
            if (right2Left) {
                mBgColor = typedArray.getColorStateList(
                        R.styleable.XXFRatingBar_starColor);
            } else {
        ...
        if (typedArray.hasValue(R.styleable.XXFRatingBar_subStarColor)) {
            if (!right2Left) {
                mSubStarColor = typedArray.getColorStateList(
                        R.styleable.XXFRatingBar_subStarColor);
            }
        }

        if (typedArray.hasValue(R.styleable.XXFRatingBar_bgColor)) {
            if (right2Left) {
                mStarColor = typedArray.getColorStateList(
                        R.styleable.XXFRatingBar_bgColor);
            } else {
                mBgColor = typedArray.getColorStateList(
                        R.styleable.XXFRatingBar_bgColor);
            }
        ...
        mKeepOriginColor = typedArray.getBoolean(R.styleable.XXFRatingBar_keepOriginColor, false);
        scaleFactor = typedArray.getFloat(R.styleable.XXFRatingBar_scaleFactor, 1);
        starSpacing = typedArray.getDimension(R.styleable.XXFRatingBar_starSpacing, 0);

        // get customize drawable
        mStarDrawable = typedArray.getResourceId(R.styleable.XXFRatingBar_starDrawable, R.drawable.ic_rating_star_solid);
        if (typedArray.hasValue(R.styleable.XXFRatingBar_bgDrawable)) {
            mBgDrawable = typedArray.getResourceId(R.styleable.XXFRatingBar_bgDrawable, R.drawable.ic_rating_star_solid);
        } else {
    ...
    public interface OnRatingChangeListener {
         ...
         */
        void onRatingChanged(XXFRatingBar ratingBar, float rating);
    }
